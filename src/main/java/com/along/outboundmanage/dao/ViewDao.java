...
public interface ViewDao {
	...
	@Cacheable(value = "getCountByDate" , key = "#area + ''+#time")
	...
	@Cacheable(value = "getDateByData"  ,key = "#area + ''+#time + ''+#type")
	@CacheExpire(expire = 60*60)
	/** 查询某时间状态占比：
	...
	@Select("select   t.type as type,count(t.pid) as count,${time} as data from (\n" +
			...
			" group by  t.type\n" +
			" order by  t.type")
	List<ViewCount> getPreByData(@Param("time") String time,@Param("type") String type, @Param("area")String area);
	 ...
	 *                  状态： type in ();
	 */
	@Cacheable(value = "getCountDataByData"  ,key = "#area + ''+#time + ''+#type")
	@CacheExpire(expire = 60*60)
	...
	//获取各地级市总人数
	@Cacheable(value = "getCountByCity")
	@CacheExpire(expire = 60*60)
	...
	//获取某个地级市各个监所总人数
	@Cacheable(value = "getSecCountByCity"  ,key = "#areaId + ''")
	@CacheExpire(expire = 60*60)
	@Select("select count(p.id) as count ,a.id as areaId, a.name as name\n" +
			" from (\n" +
	...
	//获取某个监所总人数
	@Cacheable(value = "getThrCountByCity",key = "#areaId + ''")
	@CacheExpire(expire = 60*60)
	@Select("SELECT count(p.id) as count  ,a.id as areaId ,a.name as name\n" +
			" FROM outbound_prisoner p\n" +
			" left join outbound_area a on a.id=p.area_id\n" +
	...
	//获取各地级市状态
	@Cacheable(value = "getTypeByCity",key = "#time + ''")
	@CacheExpire(expire = 60*60)
	...
	//获取某个地级市各个监所状态
	@Cacheable(value = "CountTypeByCity" ,key = "#areaId + ''+#time ")
	@CacheExpire(expire = 60*60)
	@Select("select group_concat(m.type separator '&') as type ,m.name,m.areaId  " +
			" from ( " +
			"  select CONCAT_WS(':',t.type,count(t.pid))as type ,a.name,a.id as areaId\n" +
	...
	@Cacheable(value = "getThrTypeByCity" ,key = "#areaId + ''+#time ")
	@CacheExpire(expire = 60*60)
	//获取某个监所状态
