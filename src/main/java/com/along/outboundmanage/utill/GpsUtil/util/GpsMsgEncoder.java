 ...
 * </pre>
 *
 * <h3>3 bytes length field at the end of 5 bytes header, do not strip header
 * </h3>
 *
 ...
 *
 */
public class GpsMsgEncoder extends ByteToMessageDecoder {
	...
	private static void failOnFrameLengthLessThanLengthFieldEndOffset(ByteBuf in, long frameLength,
	...
	private void exceededFrameLength(ByteBuf in, long frameLength) {
		...
		if (discard < 0) {
			// buffer contains more bytes then the frameLength so we can discard
			// all now
	...
	private static void failOnFrameLengthLessThanInitialBytesToStrip(ByteBuf in, long frameLength,
	...
	protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
		...
		for (int i = 0; i < data.length; i++) {
			//System.out.print(ConvertData.byteToHex(data[i]));
			if(data[i]==0x7D){
				blen++;
			}
		}
		...
		frameLength += lengthAdjustment + lengthFieldEndOffset+blen;
